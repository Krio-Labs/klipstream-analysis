...
RAW_CHAT_DIR.mkdir(exist_ok=True, parents=True)
...
def download_chat(video_id, output_dir=None):
    ...
    logging.info(f"Downloading chat for VOD {video_id} to {output_dir}")
...
def main(video_id=None, output_dir=None, keep_json=False):
    ...
    Args:
        ...
        keep_json (bool): Whether to keep the intermediate JSON file (not used currently)
    """
    ...
    try:
        ...
        if output_dir != DATA_DIR:
            ...
            logging.info(f'Chat data also copied to: {data_csv_path} for backward compatibility')
