...
class TwitchVideoDownloader:
    ...
    async def process_video(self, url, output_dir=None, save_to_raw=False):
        """
        ...
        Args:
            ...
            save_to_raw (bool): Whether to save files to Raw directory structure
        """
        try:
            # Get video ID and create paths
            video_id = self.extract_video_id(url)

            # Determine output paths based on parameters
            if save_to_raw:
                ...
                audio_file = Path(output_dir) / f"audio_{video_id}.wav"
            else:
            ...
            # Cleanup temporary folders but keep the video and audio files
            ...
            # Clean up temporary directories but NOT the output files
            self._cleanup_temp_folders()
            raise
